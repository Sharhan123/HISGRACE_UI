...
import { Idriver, IdriverRes, Ivehicle } from '../../interfaces';
...
const EditVehicle: React.FC<ModalProps> = ({reload,loader,data,hide}) => {
    ...
    const [errors, setErrors] = useState<{ name: string, age: string, mobile: string, exp: string, driverBata: string, image:string }>()
    const [driver, setDriver] = useState<Idriver>({
        ...
        driverBata: data.driverBata.toString(),
        mobile: data.mobile,
        image: data.image,
    ...
    const handleSubmit = async () => {
        ...
        if(!driver.image){
            setErrors({
                ...
                mobile:'',
                driverBata:'',
                image:'Please select a driver image,Its  is required'
            })
            return 
        }
        setShowModal(false)
        ...
        const editData = {
            ...
            image: driver.image,
            mobile: data.mobile,
            driverBata: parseInt(driver.driverBata),
            changed:changed
        }

        try{
            const res = await updateDriver(editData)
            hide()
            reload()
            dispatch(showAlert({head:"Hisgrace Admin Driver Updated",content:`${driver.name} 's  details has been updated to hisgrace application.`,color:'blue'}))
    ...
    const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files.length > 0) {
            ...
            reader.onload = (event) => {
                ...
                setDriver(prev=> ({...prev ,image:base64String}))
                setChanged(true)
            };
                ...
                >
                    ...
                    >
                                ...
                                driver.image ? (
                                    <div className="image-preview">
                                        ...
                                        <div className="preview-item flex items-center ">
                                            <img src={driver.image} className='max-h-48' onClick={handleRemoveImage} alt={`Preview`} />
                                            <button
                                                type="button"
                                    ...
                                    <>
                                        <label htmlFor="input-group-1" className="block mb-2 text-sm font-medium text-black text-start">Driver Images</label>
                                        <div className="relative mb-6">

                                            <input type="file" accept="image/*"
                                                onChange={handleFileChange} name='image' id="input-group-1" required className={`bg-gray-50 border ${errors?.image?'border-red-500':'border-gray-300'} text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5`} />
                                                                        <p className='text-red-500 kanit-regular text-start ml-2'>{errors?.image}</p>
                        ...
                        <footer className="flex flex-shrink-0 flex-wrap items-center justify-end flex-row p-4 gap-1" >
                            ...
                            >
                                <span className="flex items-center justify-center space-x-2">Edit Driver</span>
                            </button>
                        </footer>
