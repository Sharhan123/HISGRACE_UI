...
import UserDetails from '../customUI/userDetails';



const RecordTable: React.FC = () => {
    const [data, setdata] = useState<IuserRes[]>([])
    const [showData, setShowData] = useState<IuserRes[]>([])
    ...
    const handleSearch = (e: any) => {
        ...
        const arr = data.filter((item) => item.email.toLowerCase().includes(value) || item.name.toLowerCase().includes(value))
        setShowData(arr)
    }
            ...
            <section className="container px-4 mx-auto">
                ...
                <div className="mt-6 md:flex md:items-center md:justify-between">
                    ...
                    <div className="relative flex items-center mt-4 md:mt-0">
                        ...
                        <input type="text" onChange={(e: any) => handleSearch(e)} placeholder="Search" className="block w-full py-1.5 pr-5 text-gray-700 bg-white   rounded-lg md:w-80 placeholder-gray-400/70 pl-11 rtl:pr-11 rtl:pl-5  " />
                    </div>
                </div>
                                ...
                                <table className="min-w-full   divide-y  divide-gray-200 ">
                                    ...
                                    <tbody className="bg-white divide-y  divide-gray-500 ">
                                        {showData && (showData.map((item, index) => (
                                            <>
                                                <UserDetails user={selected} close={() => setShow(false)} showModal={show} />
                                                ...
                                                <tr key={index} >
                                                    ...
                                                    <td className="px-4 py-4 text-sm font-medium whitespace-nowrap text-center">
                                                        <div>
                                                            <h2 className="text-md kanit-regular bg-custom  rounded-md text-white">{item.email}</h2>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="px-4 py-4 text-sm   font-medium whitespace-nowrap text-center ">
                                                        <div>
                                                            <button onClick={() => {
                                                                setShow(true)
                                                                setSelected(item)
                                                            }} className={`text-md w-full h-8 rounded-md bg-gradient-to-br from-orange-600 to-orange-400 kanit-regular text-white`}>
                                                                View Details                                                    </button>
                                                        </div>
                                                    </td>
                                                </tr>
