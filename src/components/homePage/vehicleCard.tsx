  ...
  return (
    <div  className="relative overflow-hidden shadow-lg bg-white rounded-lg">
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-4 bg-black bg-opacity-50">
          ...
          <h1 onClick={() => handleClick(vehicle.id)} className=" lg:text-xl md:text-xl text-sm font-semibold text-white mb-1 kanit-bold">
            {vehicle.vehicleName}
          </h1>
          <p className="text-white mb-1 lg:text-lg  md:text-lg text-xs kanit-semibold">Premium {vehicle.seat} Seater {vehicle.type}</p>
          <p className="text-white lg:text-sm md:text-sm text-xs  kanit-light">{vehicle.desc}</p>
        <div className='mt-5'>
